// ============================================================================
//  claude-terminal-face-status.glsl — Ghostty custom shader（状態連携版）
//
...
//  最も近いキー色を距離ベースで判定する（詳細は docs/SPEC.md §9）。
...
// SDF 距離 → 輝度（コア + 外向きブルーム）。docs/SPEC.md §1.2 の式を関数化したもので、
...
// （docs/SPEC.md §7.1 と同根の問題）ので、各デコレーションは自前で輝度
...
float faceField(vec2 p, vec2 g, float t,
    ...
    if (wIdleS > 0.004) {
        ...
        //  - タイミング: 周期ごとに hash で発生位置を揺らす（docs/SPEC.md §7.2 対応）。
