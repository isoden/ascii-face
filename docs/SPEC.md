...
| ファイル                     | 役割                                                      | 状態         |
| ---------------------------- | --------------------------------------------------------- | ------------ |
| `misc/ascii-face-proto.html` | Web プロトタイプ。7 表情 + 手動切替 + オート + テーマ切替 | 動作確認済み |
| `ascii-face.glsl`            | Ghostty custom shader（基本版。入力時刻ベースの表情）     | 未実機検証   |
| `ascii-face-status.glsl`     | 上記 + exit code 連携（カーソル色サイドチャネル）         | 未実機検証   |
...
## 2. Web プロトタイプ (`misc/ascii-face-proto.html`)
...
## 3. Ghostty 移植 (`ascii-face.glsl` / `ascii-face-status.glsl`)
...
custom-shader = ~/.config/ghostty/shaders/ascii-face.glsl
...
## 5. exit code 連携（`ascii-face-status.glsl`）
...
**2026-07-14: `ascii-face-status.glsl` のみ実装済み。** 周期番号を seed にした
`hash()` で発生タイミングを周期内 0.45〜0.87 の範囲で揺らし、エンベロープも
対称ガウシアンから「閉じ 0.10s / 開き 0.24s」の非対称に変更した（まぶたの
随意収縮は速く弛緩は遅い）。`ascii-face.glsl` は未対応のまま。
...
- [ ] `ascii-face.glsl` が macOS でコンパイル・描画される
- [ ] `ascii-face.glsl` が Linux でコンパイル・描画される
- [ ] 打鍵 → SMILE、放置 → IDLE → SLEEP が意図した秒数で遷移する
- [ ] 視線がカーソルを追う（上下が逆でない）
- [ ] `ascii-face-status.glsl` で `false` 実行 → SAD にモーフし、`true` 実行 → 復帰する
...
`ascii-face-status.glsl` の「exit code 連携」（§5）を一般化し、Claude Code の作業状態
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash '~/path/to/ascii-face/claude-face-hook.sh'",
            "timeout": 5,
          },
        ],
      },
...
precmd_functions=(_ascii_face_status $precmd_functions[@])
```

### 9.7 スコープ外

Web 版（`misc/ascii-face-proto.html`）への同様の連携は今回のスコープ外。Web/GLSL 版のパーツ定義
