...
head（5 ステップ）→ body（26 ステップ）→ tail（4 ステップ + 終端 1）の 3 区間構成で、body が
...
| `thinking` | 思案顔（3 バリアント。§10.1）＋頭上に「?」が浮かび周期的に左右へスライド            | Claude Code hooks（調査/プランニング中）                               |
...
表情の具体的な形状はフェーズごとに 3 バリアントを持ち、`VAR_PERIOD` ごとにランダムに
切り替わる（§10.1）。**色とデコレーション（?／汗／キラキラ）は全バリアント共通**で、
フェーズの判別がバリアントで曖昧にならないようにしている。
...
各フェーズ（idle/think/work/done）が **3 表情**を持ち、`VAR_PERIOD`（7 秒）ごとに
`pickVar()` = `hash()` で次の表情を無作為に選ぶ。連続して同じ表情が選ばれるのも許容
...
- 12 パターンの一覧と各々の狙い、および専用の目・口パーツ（`cupEye` / `sqzEye` /
  `barEye` / `omegaMouth` / `waveMouth` など）の定義理由は
...
| フェーズ | デコレーション                                      | 定数                                 |
| -------- | --------------------------------------------------- | ------------------------------------ |
| thinking | 頭上に「?」が浮かび左右へスライド（進行方向へ傾く） | `QM_PERIOD` / `QM_MOVE` / `QM_SWING` |
